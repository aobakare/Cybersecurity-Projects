...
def decryptMystery():
    key = [8,1,6,2,10,4,5,3,7,9] # key given to us 
    file = open('mystery.txt','r') # open file to reac
    message =  file.read() # put all contents into a single string
    file.close() # close file 
    plainText = decryptMessage(key,message) # call function to decrypt using key given
    outputFile = open('mystery.dec.txt','a') # create file to append plaintext to
    outputFile.write(plainText) # write the plainText to the file 
