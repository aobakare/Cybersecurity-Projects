...
def encrypt(message: str, key: str):
    ...
    for i in range(len(messageLetters)):
        ...
        else: # for the remaining letters
            if messageLetters[i].isalpha(): # if it's a letter
                # previous letter index
                prevLetterIndex =  i-1
                # keep moving back if the previous character is not a letter 
                while not messageLetters[prevLetterIndex].isalpha():
                    prevLetterIndex = prevLetterIndex -1
                # get the previous letter 
                previousLetter = messageLetters[prevLetterIndex]
                # the new shift (key) is the previous letter 
                shift = SHIFTDICT[previousLetter]
                # get the index of that current letter in LETTERS
                num = LETTERS.find(messageLetters[i])
                # add shift to num (index of current) to calculate the total shift needed 
                calculatedShift = num + shift
                # handle the wrap-around if calculated shift is larger than the length of LETTERS
                if calculatedShift >= len(LETTERS):
                    calculatedShift = calculatedShift - len(LETTERS)
    ...
    for i in range(len(messageLetters)):
        if i == 0: # for the first letter 
            ...
            if messageLetters[i].isalpha(): # if it's a letter
                ...
                while not messageLetters[prevLetterIndex].isalpha():
                    prevLetterIndex = prevLetterIndex -1
                # get the previous letter in the ALREADY DECRYPTED message 
                previousLetter = decryptList[prevLetterIndex]
                # the new shift (key) is the previous letter 
                shift = SHIFTDICT[previousLetter]
                # get the index of that current letter in LETTERS
                num = LETTERS.find(messageLetters[i])
